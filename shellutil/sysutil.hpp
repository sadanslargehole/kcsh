...
// Attempt to get the current user's home directory. $HOME or current directory
// if nonexistent.
...
inline std::string prettyPath(std::string path) { return replaceSubstring(path, homePath(), "~"); }

inline std::string prettyPath(fs::path path) { return prettyPath(path.string()); }
